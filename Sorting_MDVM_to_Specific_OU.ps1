...
$Users = Get-ADUser -Filter {Enabled -eq $True} -Properties Description | ? {$_.Description -like "*Managing DVM*"}
...
$Report | Export-CSV -Path "C:\users\$env:username\desktop\sortinglistmdvm.csv" -NoTypeInformation